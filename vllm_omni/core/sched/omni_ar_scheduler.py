...
class OmniARScheduler(OmniSchedulerMixin, VLLMScheduler):
    ...
    def _decode_pooling_output(self, request: Any, pooler_output: Any) -> Any:
        """Apply a per-stage pooling-output decoder hook, if configured.

        A stage may set ``pooling_output_decoder`` (a dotted path) in its
        engine_args to convert a bare pooler tensor into an omni payload
        ``dict[str, Tensor]`` before IPC serialization (omni patched
        ``EngineCoreOutput.pooling_output`` to a dict, so a bare tensor would
        fail to cross the stage boundary). When unset, the output passes
        through unchanged. This keeps model-specific decode logic out of the
        generic scheduler (the forced aligner supplies its own decoder).
        """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # engine_args -> model_config). Used e.g. by the forced-aligner stage to
        # turn pooler logits into a word-timestamps payload worker-side.
        self._pooling_output_decoder = None
        _decoder_path = getattr(self.vllm_config.model_config, "pooling_output_decoder", None)
        if _decoder_path:
            import importlib

            _mod, _fn = str(_decoder_path).rsplit(".", 1)
            self._pooling_output_decoder = getattr(importlib.import_module(_mod), _fn)
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            pooling_output_payload = self._decode_pooling_output(request, pooler_output)
