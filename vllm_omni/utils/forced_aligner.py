...
class ForcedAlignerConfig:
    """Plain-data config for the forced-aligner pipeline stage.

    Each field maps onto the injected stage in
    ``utils.forced_aligner.inject_forced_aligner_stage``: ``model``/``architecture``/
    ``runner``/``dtype``/``trust_remote_code`` -> the stage's ``engine_extras``,
    ``gpu_memory_utilization``/``max_model_len`` -> its deploy config, and
    ``pooling_task`` -> ``default_pooling_params``.
    """
...
def inject_forced_aligner_stage(
    ...
    fa = build_forced_aligner_config(
        SimpleNamespace(
            forced_aligner=cli_overrides.get("forced_aligner"),
...
def extract_word_timestamps(res: Any, text: str | None = None, language: str | None = None) -> list[dict] | None:
    ...
    (``res.outputs.data`` is an int32 ``[n_words, 2]`` tensor), or ``None`` if
    this output is not from the aligner. Word strings come from ``aligner_words``
    in ``additional_information``, else from re-segmenting ``text``.
    """
    ...
    try:
        ...
        # Word strings: prefer the aligner's own, else re-segment the request text.
        words: list[str] = []
        ...
        if not words and text is not None:
            try:
        ...
        for i, pair in enumerate(ms_list):
            ...
            result.append({"word": words[i] if i < len(words) else "", "start_ms": start_ms, "end_ms": end_ms})
        return result
    except Exception:
